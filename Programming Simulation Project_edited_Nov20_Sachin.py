...
def update_simulation():
    ...
    simulation_plots[0].set_xlabel(r'Crank Angle ($\degree$)')
    simulation_plots[0].set_ylabel('Cylinder Pressure (kPa)')
    simulation_plots[0].set_ylim(0, 20000)
    simulation_plots[0].set_xlim(-360, 360)
    simulation_plots[0].set_xticks([-360,-240,-120,0,120,240,360])
    c_angle = np.linspace(-360, 359.95, len(states.P)).tolist() # hard-coded crank angle 
    simulation_plots[0].plot(c_angle, states.P/1000)
...
injection_time_slider = tk.Scale(root, label="Adjust Injection Timing", from_=1, to=10, orient="vertical")
injection_time_slider.grid(row=5, column=0, rowspan=3)
